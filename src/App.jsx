...
function App() {
  ...
  useEffect(() => {
    ...
    console.log('🚀 Initializing socket on app mount...', { isGuest, hasToken: !!token });
    ...
    // Intent-Aware Collaboration: listen for intent updates from other users
    const handleIntentUpdate = (data) => {
      setUserIntents(prev => ({
        ...prev,
        [data.socketId]: data.intent?.key || data.intent
      }));
    };
    const unsubIntentUpdate = onIntentUpdate(handleIntentUpdate);
  ...
  const handleJoinRoom = useCallback((roomId) => {
    ...
    // Get token based on current mode (guest or auth)
    const params = new URLSearchParams(window.location.search);
    const isGuest = params.get('guest') === 'true';
    const token = isGuest ? null : localStorage.getItem('token');

    // Initialize socket if not connected (or reconnect with correct auth)
    initSocket(token);
