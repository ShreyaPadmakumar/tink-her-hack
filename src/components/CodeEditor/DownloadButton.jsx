    ...
    return (
        <div className="relative" ref={dropdownRef}>
            <button
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 bg-[#222225] hover:bg-[#2a2a30] rounded-lg transition-colors duration-200"
                title="Download code"
            >
                <Download className="w-4 h-4 text-[#7a7a80]" />
                <ChevronDown className={`w-3 h-3 text-[#4e4e56] transition-transform duration-200 ${isOpen ? 'rotate-180' : ''}`} />
            </button>

            {/* Dropdown Menu */}
            {isOpen && (
                <div className="absolute top-full right-0 mt-1 w-48 bg-[#1a1a1e] border border-[#222225] rounded-lg shadow-xl z-50 overflow-hidden animate-fadeIn">
                    <button
                        onClick={downloadAsSource}
                        className="w-full text-left px-3 py-2.5 text-sm flex items-center gap-2 text-[#ededef] hover:bg-[#222225] transition-colors"
                    >
                        <FileCode className="w-4 h-4 text-[#6e7bf2]" />
                        <span>Download as {ext}</span>
                    </button>
                    <div className="border-t border-[#222225]"></div>
                    <button
