...
export default function TabBar({
    ...
    const renderTab = (tab, groupColor = null) => {
        ...
        return (
            <div
                ...
                className={`group relative flex items-center gap-2 px-4 py-2 cursor-grab transition-all duration-150 border-r border-[#222225] min-w-[120px] max-w-[200px] select-none
          ${isActive ? 'bg-[#1a1a1e]' : 'bg-transparent hover:bg-[#141416]'}
          ${isDragging ? 'opacity-50 bg-[#222225]' : ''}
          ${isDropTarget ? 'bg-[#6e7bf2]/10' : ''}`}
                style={colorValue ? {
                    borderTop: `2px solid ${colorValue}`,
                    backgroundColor: isActive ? `${colorValue}15` : isDragging ? '#222225' : undefined
                } : undefined}
            >
                ...
                {/* Active indicator */}
                {isActive && !groupColor && (
                    <div className="absolute bottom-0 left-0 right-0 h-[2px] bg-[#6e7bf2]"></div>
                )}

                {/* Drag handle (subtle) */}
                <GripVertical className="w-3 h-3 text-[#2a2a30] opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0" />
                ...
                {/* Filename */}
                <span className={`text-sm truncate ${isActive ? 'text-[#ededef]' : 'text-[#7a7a80]'}`}>
                    {tab.filename}
                </span>
              ...
              ${isActive ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'}`}
                        title="Close tab"
                    >
                        <X className="w-3.5 h-3.5 text-[#7a7a80] hover:text-[#e5484d]" />
                    </button>
                )}
    ...
    return (
        <div className="relative flex items-end bg-[#141416] border-b border-[#222225] overflow-x-auto scrollbar-thin">
            ...
            {activeGroups.map(group => {
                ...
                return (
                    <div
                        key={group.id}
                        className={`flex items-end transition-all duration-150 ${isGroupDragOver ? 'bg-[#6e7bf2]/10 rounded-t' : ''
                            }`}
                    ...
                    >
                        ...
                        {group.collapsed && (
                            <div
                                className="px-2 py-1 text-xs text-[#4e4e56] border-r border-[#222225]"
                        ...
                        {isGroupDragOver && draggedTab && !groupTabs.find(t => t.id === draggedTab.tab.id) && (
                            <div className="px-2 py-1 text-xs text-[#6e7bf2] animate-pulse">
                                + Add to {group.name}
                            </div>
                        )}
                    </div>
            ...
            <button
                onClick={onNewTab}
                className="flex items-center justify-center w-9 h-9 hover:bg-[#141416] transition-colors flex-shrink-0"
                title="New file (Ctrl+N)"
            >
                <Plus className="w-4 h-4 text-[#7a7a80] hover:text-[#ededef]" />
            </button>
