...
export default function NewFileModal({ isOpen, onClose, onCreate, existingFilenames = [] }) {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
            ...
            {/* Modal */}
            <div className="relative bg-[#141416] border border-[#222225] rounded-xl shadow-2xl w-full max-w-md mx-4 animate-fadeIn">
                {/* Header */}
                <div className="flex items-center justify-between px-5 py-4 border-b border-[#222225]">
                    <div className="flex items-center gap-2">
                        <FileCode className="w-5 h-5 text-[#6e7bf2]" />
                        <h2 className="text-lg font-semibold text-[#ededef]">Create New File</h2>
                    </div>
                    <button
                        onClick={handleClose}
                        className="p-1 hover:bg-[#222225] rounded-lg transition-colors"
                    >
                        <X className="w-5 h-5 text-[#7a7a80]" />
                    </button>
                </div>
                ...
                <form onSubmit={handleSubmit} className="p-5 space-y-5">
                    ...
                    <div>
                        <label className="block text-sm text-[#7a7a80] mb-3">
                            Filename
                        </label>
                        <div className="flex items-center gap-3">
                            <input
                                ...
                                className="flex-1 px-4 py-3 bg-[#1a1a1e] border border-[#222225] rounded-lg text-[#ededef] placeholder-[#4e4e56] focus:outline-none focus:border-[#6e7bf2] transition-colors text-lg"
                                autoFocus
                            />
                        ...
                        {error && (
                            <p className="mt-2 text-sm text-[#e5484d]">{error}</p>
                        )}
                    </div>
                    ...
                    ${language === lang.id
                                            ...
                                            : 'bg-[#1a1a1e] border-[#222225] text-[#7a7a80] hover:border-[#4e4e56]'
                                        }`}
                    ...
                    {filename && (
                        <div className="p-4 bg-[#141416] rounded-lg border border-[#222225]">
                            <div className="flex items-center gap-3">
                                <FileCode className="w-6 h-6" style={{ color: selectedLang.color }} />
                                <div>
                                    <p className="text-xs text-[#4e4e56] mb-1">Will create:</p>
                    ...
                    <div className="flex justify-end gap-3 pt-2">
                        <button
                            type="button"
                            onClick={handleClose}
                            className="px-5 py-2.5 text-sm text-[#7a7a80] hover:text-[#ededef] hover:bg-[#222225] rounded-lg transition-colors"
                        >
                            Cancel
                        </button>
                        <button
                            type="submit"
                            className="px-5 py-2.5 text-sm bg-[#6e7bf2] hover:bg-[#8b96f7] text-[#141416] font-semibold rounded-lg transition-colors"
                        >
                            Create File
                        </button>
                    </div>
